...
class ProfileViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        client.run(Accounts.account(id: profile_id)) { (result) in
            let profile = result.value
            if let profile = profile {
                self.account = profile
                
                DispatchQueue.main.async(execute: {
                    self.client.run(Accounts.currentUser(), completion: { (result) in
                        let own_account = result.value
    ...
    func downloadToots() {
        client.run(Accounts.statuses(id: profile_id)) { (result) in
    ...
    func getFollowed() {
        client.run(Accounts.currentUser()) { (result) in
            ...
            if let user = user {
                self.client.run(Accounts.following(id: user.id), completion: { (result) in
    ...
    func getIsFollowing() {
        client.run(Accounts.currentUser()) { (result) in
            let user = result.value
            if let user = user {
                self.client.run(Accounts.following(id: self.profile_id), completion: { (result) in
                    let followers = result.value
                    if let followers = followers {
                        var follows = false
                        for follower in followers {
    ...
    @IBAction func followButton(_ sender: UIButton) {
        ...
        if self.following {
            client.run(Accounts.follow(id: profile_id)) { (result) in
            ...
            client.run(Accounts.unfollow(id: profile_id)) { (result) in
                let error = result.error
                if error != nil {
