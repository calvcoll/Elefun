    ...
    @objc func sendToot (sender: Any?) {
        if self.mediaBlocks.isEmpty {
            if (self.charCount <= 500 && self.charCount > 0) {
                client.run(Statuses.create(status: tootText.text!, replyToID: replyID, mediaIDs: mediaIDs, spoilerText: spoilerText, visibility: visibilities[visibilityPicker.selectedRow(inComponent: 0)]), completion: { (result) in
...
extension TootViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        if let pickedImage = info[UIImagePickerControllerOriginalImage] as! UIImage? {
            ...
            client.run(Media.upload(media: .png(imageData)), completion: { result in
                let attachment = result.value
                let error = result.error
