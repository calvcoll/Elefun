...
//    
    ...
    func downloadContext(originalStatus: Status) {
        client.run(Statuses.context(id: originalStatus.id)) { (result) in
    ...
    @IBAction func heartButtonPressed(_ sender: Any) {
        client.run(Statuses.favourite(id: status.id)) { (result) in
            let error = result.error
            if error != nil {
    ...
    @IBAction func retootButtonPressed(_ sender: Any) {
        client.run(Statuses.reblog(id: status.id)) { (result) in
            let error = result.error
            if error != nil {
